...
contract DeployPoolTokenTestnet is Script {
    ...
    function run() public {
        ...
        rentToken.mint(address(deployer), 2 ** 255);
        Currency rentCurrency = Currency.wrap(address(rentToken));

        // Deploy the arbiter hook with required parameters
        ArbiterAmAmmAnyERC20Hook arbiterHook = new ArbiterAmAmmAnyERC20Hook(
            ICLPoolManager(address(clPoolManager)),
            ICLPositionManager(address(clPositionManager)),
        ...
        console.log("rentCurrency: ", Currency.unwrap(rentCurrency));
